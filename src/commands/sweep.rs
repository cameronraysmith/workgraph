...
mod tests {
    ...
    fn setup_graph_and_registry(dir: &Path, tasks: Vec<Task>, agents: Vec<(&str, &str, u32, AgentStatus)>) {
    ...
    fn test_sweep_no_orphans() {
        ...
        let mut graph = WorkGraph::new();
        graph.add_node(Node::Task(make_task("open-task", "Open Task", Status::Open)));
        graph.add_node(Node::Task(make_task("done-task", "Done Task", Status::Done)));
        save_graph(&graph, &gpath).unwrap();
